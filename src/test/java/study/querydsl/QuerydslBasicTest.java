...
//                .fetchOne();
    ...
    @Test
    public void paging2() {
        // 페이징 쿼리가 단순하면 fetchResults()를 쓰지만, 복잡한 경우에는 간략하게 해서 count 쿼리를 따로 구분한다.
        QueryResults<Member> memberQueryResults = queryFactory
                .selectFrom(member)
                .orderBy(member.username.desc())
                .offset(1)
    ...
    public void group() {
        List<Tuple> result = queryFactory
                ...
                .groupBy(team.name) // having도 가능하다.
                .fetch();
...
//                .join(member.team, team)
        ...
        assertThat(result)
                .extracting("username") // list에서 username 추출
                .containsExactly("member1", "member2"); // 추출한 username에 포함여부 확인
    }
...
//                .on(team.name.eq("teamA"))
    ...
    public void fetchJoinUse() {
        ...
        Member findMember = queryFactory
                .selectFrom(member)
                .join(member.team, team).fetchJoin() // <- fetchJoin : 연관된 앤티티를 한번에 조회
                .where(member.username.eq("member1"))
                .fetchOne();
    ...
    public void selectSubQuery() {
        ...
        /**
         *  JPA, JPQL 서브쿼리의 한계는 from절의 서브쿼리는 지원하지 않는다.
         */
